<template>
    <v-card width="30%" class="rounded-xl" elevation="0">
        ...
        <v-row>
            <v-card-title class="mx-auto mb-2 header">{{ this.$data.user.username}}</v-card-title>
        </v-row>
        <v-row>
            <v-sheet class="mx-auto mb-4 rounded-lg" width="85%" color="secondary darken-3">
                <div class="mx-3 mt-1">
                    <v-row class="secondary darken-2 rounded-lg mb-3">
                        <h3 v-if="this.$data.message.author.id == this.$store.state.userId" class="ml-2 colored-header">You:</h3>
                        <h3 v-else class="ml-2 colored-header">Them:</h3>
                    </v-row>
                    <p>{{message.content.substring(0,110)}}{{message.content.length >= 110 ? '...' : ''}}</p>
                </div>
            </v-sheet>
        </v-row>
    </v-card>
