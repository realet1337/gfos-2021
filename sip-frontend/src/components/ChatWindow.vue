...
export default {
    ...
    methods: {
        ...
        updateMessages: function(before, after){
            ...
            window.axios.get(Vue.prototype.$getApiUrl('http') + '/chats/' + this.$route.params.chatId + '/chat-messages', {
                headers:{
        ...
        sendMessage: function(){
            if(/\S/.test(this.$data.message)){
                if(this.$data.editing.id){
                    window.axios.put(Vue.prototype.$getApiUrl('http') + '/chat-messages',
                    {
                    ...
                    window.axios.post(Vue.prototype.$getApiUrl('http') + '/chats/' + this.$route.params.chatId + '/chat-messages',
                    {
                            //removing leading and trailing whitespaces
                            content: this.$data.message.match(/\S(.*\S)?/s)[0],
                    },
        ...
        createWatcher: function(){
            //create watcher
            var _this = this;
            var ws = new WebSocket(Vue.prototype.$getApiUrl('ws') + '/chats/' + this.$route.params.chatId + '/websockets');
            ws.onopen = function(){
