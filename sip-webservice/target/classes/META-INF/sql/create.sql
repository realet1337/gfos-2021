...
CREATE TABLE Roles (id INT UNSIGNED AUTO_INCREMENT NOT NULL, group_id INT UNSIGNED NOT NULL, role_name VARCHAR(64) NOT NULL, role_color VARCHAR(7) NOT NULL, administrator BOOLEAN NOT NULL, FOREIGN KEY (group_id) REFERENCES `Groups`(id), PRIMARY KEY (id)); 
...
CREATE TABLE Permissions(id INT UNSIGNED AUTO_INCREMENT NOT NULL, role_id INT UNSIGNED NOT NULL, chat_id INT UNSIGNED NOT NULL, canRead BOOLEAN NOT NULL, canWrite BOOLEAN NOT NULL, FOREIGN KEY (role_id) REFERENCES Roles(id), FOREIGN KEY (chat_id) REFERENCES Chats(id), PRIMARY KEY(id));
